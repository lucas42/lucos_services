		...
		try {
			...
			try {
				...
				while ((headerLine = br.readLine()) != null) {
					...
					// Don't forward any headers starting with X-
					if (headerLine.startsWith("X-")) continue;
				...
				// Add a header to indicate where the requeest originated from
				request.add("X-Forwarded-For: " + socket.getInetAddress().getHostAddress());
				if (host == null) {
				...
				if (port == Manager.getPort()) {
					...
					Manager.log("Host: "+host+", Port: "+port+", Request: "+request.peek());
					Socket serversocket = new Socket(InetAddress.getLocalHost(), port);
					final InputStream serveris = new DataInputStream(serversocket.getInputStream());
					BufferedReader serverbr = new BufferedReader(new InputStreamReader(serveris, "UTF8"));
					final PrintStream serverout = new PrintStream( serversocket.getOutputStream());
					
					// Write all the request request to the server
					Iterator requestiter = request.iterator();
					while (requestiter.hasNext()) {
						String requestLine = (String)requestiter.next();
						...
						public void run() {
							try {
								...
								try {
									...
									Manager.logErr("Error Closing streams and socket (server thread exception)");
									Manager.logErr(ee);
								}
							} catch (Exception e) {
								Manager.logErr("Server Thread Error:");
								Manager.logErr(e);
							}
						}
					}).start();
					...
					new Thread(new Runnable() {
						public void run() {
							try {
								byte[] buffer = new byte[1024];
								int bytesRead;
								while ((bytesRead = serveris.read(buffer)) != -1) {
								...
								try {
									...
									Manager.logErr("Error Closing streams and socket (client thread)");
									Manager.logErr(e);
								}			
							} catch (IOException e) {
								Manager.logErr("Connection to Client Error:");
								Manager.logErr(e);
								try {
									...
									Manager.logErr("Error Closing streams and socket (client thread exception)");
									Manager.logErr(ee);
								}		
								
							} catch (Exception e) {
								Manager.logErr("Client Thread Error:");
								Manager.logErr(e);
							}
						}
					}).start();
